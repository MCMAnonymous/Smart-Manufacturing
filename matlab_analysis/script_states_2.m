...
[x,y] = find(ini_states(:,1) >= time_ini & ini_states(:,1) <= time_end);

fila_states = x;

RC = 1;

stem(ini_states(fila_states,RC), 10000000*ones(length(ini_states(fila_states,RC)),1) );
stem(ini_states_idle(fila_states,RC), 10000000*ones(length(ini_states_idle(fila_states,RC)),1) );
...
% if ~isempty(x)
%     length(x)
%     stem(s7_busy(x(1),:).TimeStamp-msg_time_aux(1), 5000);
...
% if ~isempty(x)
%     length(x)
%     stem(sH_moving(x(1),:).TimeStamp-msg_time_aux(1), 5000);
%     % stem(s7_idle(x(1),:).TimeStamp-msg_time_aux(1), 5000);
% end